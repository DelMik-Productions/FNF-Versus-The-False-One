...
typedef MusicSpriteGroup = MusicTypedSpriteGroup<MusicSprite>;

class MusicTypedSpriteGroup<T:MusicSprite> extends MusicSprite
{
	/**
	 ...
	 */
	var _sprites:Array<MusicSprite>;
	...
	override function onUpdate(songPosition:Float):Void
	{
		...
		forEach(v -> v.onUpdate(songPosition));
	}

	override function onTick(tick:Int):Void
	{
		this.tick = tick;
		forEach(v -> v.onTick(tick));
	}

	override function onBeatHit(beat:Int):Void
	{
		curBeat = beat;
		forEach(v -> v.onBeatHit(beat));
	}

	override function onStepHit(step:Int):Void
	{
		curStep = step;
		forEach(v -> v.onStepHit(step));
	}
	 ...
	 */
	override public function clone():MusicTypedSpriteGroup<T>
	{
		var newGroup = new MusicTypedSpriteGroup<T>(x, y, maxSize);
	...
	{
		var sprite:MusicSprite = cast Sprite;
		sprite.x += x;
		sprite.y += y;
		sprite.alpha *= alpha;
		sprite.scrollFactor.copyFrom(scrollFactor);
		sprite.cameras = _cameras; // _cameras instead of cameras because get_cameras() will not return null
		sprite.playState = playState;
	...
	function findMinXHelper()
	{
		...
		for (member in _sprites)
		{
			...
			if (member.flixelType == SPRITEGROUP)
				minX = (cast member : MusicSpriteGroup).findMinX();
			else
				minX = member.x;
	...
	function findMaxXHelper()
	{
		...
		for (member in _sprites)
		{
			...
			var maxX:Float;
			if (member.flixelType == SPRITEGROUP)
				maxX = (cast member : MusicSpriteGroup).findMaxX();
			else
	...
	function findMinYHelper()
	{
		...
		for (member in _sprites)
		{
			...
			var minY:Float;
			if (member.flixelType == SPRITEGROUP)
				minY = (cast member : MusicSpriteGroup).findMinY();
			else
				minY = member.y;
	...
	function findMaxYHelper()
	{
		...
		for (member in _sprites)
		{
			...
			var maxY:Float;
			if (member.flixelType == SPRITEGROUP)
				maxY = (cast member : MusicSpriteGroup).findMaxY();
			else
	...
	inline function xTransform(Sprite:MusicSprite, X:Float)
		Sprite.x += X; // addition

	inline function yTransform(Sprite:MusicSprite, Y:Float)
		Sprite.y += Y; // addition

	inline function angleTransform(Sprite:MusicSprite, Angle:Float)
		Sprite.angle += Angle; // addition

	inline function alphaTransform(Sprite:MusicSprite, Alpha:Float)
	{
	...
	inline function directAlphaTransform(Sprite:MusicSprite, Alpha:Float)
		Sprite.alpha = Alpha; // direct set

	inline function facingTransform(Sprite:MusicSprite, Facing:Int)
		Sprite.facing = Facing;

	inline function flipXTransform(Sprite:MusicSprite, FlipX:Bool)
		Sprite.flipX = FlipX;

	inline function flipYTransform(Sprite:MusicSprite, FlipY:Bool)
		Sprite.flipY = FlipY;

	inline function movesTransform(Sprite:MusicSprite, Moves:Bool)
		Sprite.moves = Moves;

	inline function pixelPerfectTransform(Sprite:MusicSprite, PixelPerfect:Bool)
		Sprite.pixelPerfectRender = PixelPerfect;

	inline function gColorTransform(Sprite:MusicSprite, Color:Int)
		Sprite.color = Color;

	inline function blendTransform(Sprite:MusicSprite, Blend:BlendMode)
		Sprite.blend = Blend;

	inline function immovableTransform(Sprite:MusicSprite, Immovable:Bool)
		Sprite.immovable = Immovable;

	inline function visibleTransform(Sprite:MusicSprite, Visible:Bool)
		Sprite.visible = Visible;

	inline function activeTransform(Sprite:MusicSprite, Active:Bool)
		Sprite.active = Active;

	inline function solidTransform(Sprite:MusicSprite, Solid:Bool)
		Sprite.solid = Solid;

	inline function aliveTransform(Sprite:MusicSprite, Alive:Bool)
		Sprite.alive = Alive;

	inline function existsTransform(Sprite:MusicSprite, Exists:Bool)
		Sprite.exists = Exists;

	inline function cameraTransform(Sprite:MusicSprite, Camera:FlxCamera)
		Sprite.camera = Camera;

	inline function camerasTransform(Sprite:MusicSprite, Cameras:Array<FlxCamera>)
		Sprite.cameras = Cameras;

	inline function offsetTransform(Sprite:MusicSprite, Offset:FlxPoint)
		Sprite.offset.copyFrom(Offset);

	inline function originTransform(Sprite:MusicSprite, Origin:FlxPoint)
		Sprite.origin.copyFrom(Origin);

	inline function scaleTransform(Sprite:MusicSprite, Scale:FlxPoint)
		Sprite.scale.copyFrom(Scale);

	inline function scrollFactorTransform(Sprite:MusicSprite, ScrollFactor:FlxPoint)
		Sprite.scrollFactor.copyFrom(ScrollFactor);

	inline function clipRectTransform(Sprite:MusicSprite, ClipRect:FlxRect)
	{
		if (ClipRect == null)
	 ...
	 */
	override public function loadGraphicFromSprite(Sprite:FlxSprite):MusicSprite
	{
		#if FLX_DEBUG
		throw "This function is not supported in MusicSpriteGroup";
		#end
		return this;
	}
	...
	override public function loadRotatedGraphic(Graphic:FlxGraphicAsset, Rotations:Int = 16, Frame:Int = -1, AntiAliasing:Bool = false,
			AutoBuffer:Bool = false, ?Key:String):MusicSprite
	{
		#if FLX_DEBUG
		throw "This function is not supported in MusicSpriteGroup";
		#end
	 ...
	 * @return this sprite group
	 */
	override public function makeGraphic(Width:Int, Height:Int, Color:Int = FlxColor.WHITE, Unique:Bool = false, ?Key:String):MusicSprite
	{
		#if FLX_DEBUG
		throw "This function is not supported in MusicSpriteGroup";
		#end
